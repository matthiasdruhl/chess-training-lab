...
interface BlunderQueueListProps {
  items: PersonalBlunder[];
  selectedId?: string;
...
export function BlunderQueueList({
  items,
  selectedId,
  onSelect,
  onDelete,
}: BlunderQueueListProps) {
  if (items.length === 0) {
    return (
      <p className="text-sm text-slate-400">
        No blunders in queue — run a scan from the Scan tab or Dashboard.
      </p>
    );
  }
        ...
        >
          ...
          >
            ...
            <span className="text-xs text-slate-500">
              {new Date(item.source.playedAt).toLocaleDateString()} · swing{' '}
              {item.analysis.swingCp} cp · {item.analysis.classification}
            </span>
          </button>
          <span
